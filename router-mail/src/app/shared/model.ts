...
import { map, first, mergeMap, skip, scan } from 'rxjs/operators';
...
export class Repo {
  ...
  conversation(id: number): Observable<Conversation> {
    return this.state.pipe(
      mergeMap(s => s.conversations.filter(c => c.id === id)),
      first()
    );
  }
  ...
  message(id: number): Observable<Message> {
    return this.state.pipe(
      mergeMap(s => s.messages.filter(m => m.id === id)),
      first()
    );
  }
  ...
  private setUpEffects(actions: Actions) {
    ...
    zip(this.state.pipe(skip(1)), actions).subscribe(p => p[1].onSuccess(p[0]));
  }
